...
module.exports = function (req, res) {
  ...
  Ethernet.findOne({
    ...
    for (var property in req.body.ethernets) {
      if (req.body.ethernets.hasOwnProperty(property)) {
        ...
        if (property == 'name') {
          ...
          && req.body.ethernets[property].hasOwnProperty('operational')) {
          ...
          valid_changed_options[property] = req.body.ethernets[property];
        }
      }
    }
    ...
    for (var property in req.body.ethernets) {
      if (req.body.ethernets.hasOwnProperty(property)) {
        doc[property] = req.body.ethernets[property];
      }
    }

    // Run the field validations.
    Ethernet.validate(doc, function (error, api_errors) {
      if (error) {
        logger.error(error, {
          module: 'interfaces/ethernets',
          tags  : [
      ...
      if (Object.keys(valid_changed_options).length == 1
        ...
        doc.update(req.body.ethernets, function (error) {
          if (error) {
            logger.error(error, {
              module: 'interfaces/ethernets',
              tags  : [
      ...
      ip_link.set(ip_link_doc, function (error) {
        ...
        ip_link.show({
          ...
          if (!req.body.ethernets.hasOwnProperty('status')) {
            req.body.ethernets.status = {};
          }
          req.body.ethernets.status.operational = links[0].state;
