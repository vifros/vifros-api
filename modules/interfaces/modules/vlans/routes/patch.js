...
module.exports = function (req, res) {
  ...
  VLAN.findOne({
    ...
    if (doc) {
       ...
       */
      var schema_vars = JSON.parse(JSON.stringify(VLAN.schema.paths)); // This construction is to do a deep copy.
      ...
      for (var i = 0, j = Object.keys(schema_vars).length;
        ...
        if (!doc_patch.vlans[0].hasOwnProperty(key)) {
          doc_patch.vlans[0][key] = null;
        }
      }
       ...
       */
      for (var i = 0, j = Object.keys(doc_patch.vlans[0]).length;
           i < j;
           i++) {

        var key = Object.keys(schema_vars)[i];

        if (doc_patch.vlans[0][key] == null) {
          delete doc_patch.vlans[0][key];
        }
      }
      ...
      for (var i = 0, j = req.body.length;
        ...
        var path = req.body[i].path.split('/vlans/0/')[1];
       ...
       */
      doc_patch.vlans[0].dev = doc_patch.vlans[0].interface + '.' + doc_patch.vlans[0].tag;
      doc_patch.vlans[0].state = doc_patch.vlans[0].status.admin;

      // Update the interface in the OS.
      ip_link.set(doc_patch.vlans[0], function (error) {
        if (error) {
          logger.error(error, {
            module: 'interfaces/vlans',
            tags  : [
